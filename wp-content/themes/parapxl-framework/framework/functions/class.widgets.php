...
* @version	  1.6
*/
	if ( ! class_exists( 'pxlWidget' ) ) :
		class pxlWidget extends WP_Widget {
			/** 
			* Create Widget
			* 
			...
			*/
			function create_field( $key, $out = "" ) {
				/* Set Defaults */				$key['std'] = isset( $key['std'] ) ? $key['std'] : "";
				$slug = $key['id'];
				...
				/* Set field id and name  */				$key['_id'] = $this->get_field_id( $slug );
				$key['_name'] = $this->get_field_name( $slug );
				
				/* Set field type */				if ( ! isset( $key['type'] ) ) $key['type'] = 'text';
				
				/* Prefix method */				$field_method = 'create_field_' . str_replace( '-', '_', $key['type'] );
				
				/* Check for styles */
					$style = ( isset( $key['style'] ) ? " style='{$key['style']}'" : '');
				
				/* Check for <p> Class */				$p = ( isset( $key['class-p'] ) ) ? '<p class="'.$key['class-p'].'"'.$style.'>' : "<p$style>"; 
				
				/* Run method */				if ( method_exists( $this, $field_method ) ) 
			...
			*/
			function create_field_select_group( $key, $out = "" ) {
				...
				if ( isset( $key['class'] ) ) $out .= 'class="' . esc_attr( $key['class'] ) . '" ';
			...
			*/
			function create_field_number( $key, $out = "" ) {
				...
				$out .= '<input type="number" ';
 					if ( isset( $key['class'] ) ) $out .= 'class="' . esc_attr( $key['class'] ) . '" ';
					if( isset($key['min']) ) $out .= "min={$key['min']} ";
					if( isset($key['max']) ) $out .= "max={$key['max']} ";
					$value = isset( $key['value'] ) ? $key['value'] : $key['std'];
					$out .= 'id="' . esc_attr( $key['_id'] ) . '" name="' . esc_attr( $key['_name'] ) . '" value="' . esc_attr__( $value ) . '" ';
					if ( isset( $key['size'] ) ) $out .= 'size="' . esc_attr( $key['size'] ) . '" ';
				$out .= ' />';
				if ( isset( $key['desc'] ) ) $out .= '<br/><small class="description">'.esc_html( $key['desc'] ).'</small>';
				return $out;
			}
