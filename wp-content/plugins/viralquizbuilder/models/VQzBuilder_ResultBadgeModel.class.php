...
class VQzBuilder_ResultBadgeModel extends VQzBuilder_ModelCore
{

    var $delete_priority = 3;
    ...
    public static function getInstance()
    {
        ...
        {
            self::$instance = new VQzBuilder_ResultBadgeModel;
        }
    ...
    public function __construct()
    {
        ...
        $this->setTableName($this->table_prefix . 'result_badges');
    }
	...
	`text` TEXT,
	`text_color` CHAR(10),
	`text_font_id` INTEGER UNSIGNED,
	`text_fontsize_id` INTEGER UNSIGNED,
	`background_color` CHAR(10),
	`background_image` CHAR(200),
    ...
    public function add_foreign_keys()
    {
	    ...
	    ADD CONSTRAINT `FK_vqzb_resbadge_quiz` FOREIGN KEY `FK_vqzb_resbadge_quiz` (`quiz_id`)
	    REFERENCES `{$oquiz->getTableName()}` (`id`)
	    ON DELETE CASCADE
	    ON UPDATE CASCADE,
	    ADD INDEX `abc_type_answer_id`(`abc_type_answer_id`),
	    ADD CONSTRAINT `FK_vqzb_resbadge_abcanswer` FOREIGN KEY `FK_vqzb_resbadge_abcanswer` (`abc_type_answer_id`)
    ...
    public function get_one($id)
    {
		...
		rb.`background_color`,
		rb.`background_image`,
		rb.`share_text_twitter`,
		rb.`share_text_fb`,
		rb.`share_text_pin`,
		(SELECT abc.`answer` FROM `{$oabc_type_answer->getTableName()}` abc WHERE rb.`abc_type_answer_id` = abc.`id`) as `abc_type_answer_name`
		FROM `{$this->getTableName()}` rb 
		WHERE rb.`id` = {$id}", ARRAY_A);
    }
    ...
    /**
     * Check if the default badge for the quiz already exists
    ...
    {
        $quiz_id = intval($quiz_id);
        $res = $this->wpdb->get_results("SELECT b.`id` 
		FROM {$this->getTableName()} b
		WHERE b.`quiz_id` = {$quiz_id} 
		AND b.`is_default` = 1", ARRAY_A);
    ...
    public function can_create($quiz_id, $id = NULL)
    {
        ...
        {
            case 'abc-quiz':
                if (count(VQzBuilder_ABCtypeAnswerModel::getInstance()->get_not_used_in_quiz_badge($quiz_id, $id)) > 0)
                    return TRUE;
                return FALSE;
            case 'points-quiz':
            default:
    ...
    public function validate($data, $quiz_id, $id = NULL)
    {
        ...
        //validate text color
            if (isset($data['text_color']) && $data['text_color'] != '' && !vqzb_is_color($data['text_color']))
                $errors[] = "Not valid text color.";

        //validate background color
        if (isset($data['background_color']) && $data['background_color'] != '' && !vqzb_is_color($data['background_color']))
            $errors[] = "Not valid background color.";
